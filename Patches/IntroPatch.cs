...
class BeginCrewmatePatch
{
    ...
    public static void Postfix(IntroCutscene __instance, ref Il2CppSystem.Collections.Generic.List<PlayerControl> teamToDisplay)
    {
        ...
        {
            __instance.TeamTitle.text = "纪念永久的TOHE";
            __instance.ImpostorText.gameObject.SetActive(true);
            __instance.ImpostorText.text = "咔皮呆 再见";
    ...
    public static void Postfix(IntroCutscene __instance)
    {
        ...
        if (AmongUsClient.Instance.AmHost)
        {
            if (Main.NormalOptions.MapId != 4)
            {
                ...
                if (Options.FixFirstKillCooldown.GetBool() && Options.CurrentGameMode != CustomGameMode.SoloKombat && Options.CurrentGameMode != CustomGameMode.HotPotato)
                    new LateTask(() =>
                    {
            ...
            if (Options.RandomSpawn.GetBool() || Options.CurrentGameMode == CustomGameMode.HotPotato)
            {
                RandomSpawn.SpawnMap map;
                switch (Main.NormalOptions.MapId)
                {
