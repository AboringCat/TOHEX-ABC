...
internal class ChatCommands
{
    ...
    public static bool Prefix(ChatController __instance)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(CustomRoles.Chatty) && PlayerControl.LocalPlayer.IsAlive())
        {
            Main.ChattyMax[PlayerControl.LocalPlayer.PlayerId]++;
            if (Main.ChattyMax[PlayerControl.LocalPlayer.PlayerId] >= Options.ChattyNumWin.GetInt())
            {
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Chatty);
        ...
        if (AmongUsClient.Instance.AmHost || PlayerControl.LocalPlayer.FriendCode.GetDevUser().IsDev)// || player.FriendCode.GetDevUser().IsDev)
        {
            Main.isChatCommand = true;
            switch (args[0])
            {
                ...
                case "/myrole":
                    ...
                    if (GameStates.IsInGame)
                    {
                        ...
                        if (role is CustomRoles.Backup)
                            sb.Append($"\n\n" + GetString($"Honmei") + Utils.GetRoleMode(CustomRoles.Honmei) + GetString($"HonmeiInfoLong"));
                ...
                case "/s":
                    canceled = true;
                    if (args.Length > 1 && AmongUsClient.Instance.AmHost)
                ...
                case "/exe":
                    ...
                    var player1 = Utils.GetPlayerById(id);
                    if (player1 != null)
                    {
                        player1.Data.IsDead = true;
                        Main.PlayerStates[player1.PlayerId].deathReason = PlayerState.DeathReason.etc;
                        player1.RpcExileV2();
                        Main.PlayerStates[player1.PlayerId].SetDead();
                        if (player1.AmOwner) Utils.SendMessage(GetString("HostKillSelfByCommand"), title: $"<color=#ff0000>{GetString("DefaultSystemMessageTitle")}</color>");
                        else Utils.SendMessage(string.Format(GetString("Message.Executed"), player1.Data.PlayerName));
                    }
                    break;
    ...
    public static string FixRoleNameInput(string text)
    {
        ...
        return text switch
        {
            ...
            "法醫" => GetString("Doctor"),
            "撅暮者" or "撅暮" or "掘墓" or "掘墓者" => GetString("Detective"),
            "幸運兒" or "幸运" => GetString("Luckey"),
            "吉祥物" or "Mascot" => GetString("Mascot"),
            ...
            "倒霉蛋" or "倒霉熊" or "倒霉" => GetString("UnluckyEggs"),
            "赌博" or "赌博者" => GetString("EvilGambler"),
            "命运眷顾者" or "运神" or "命运" or "眷顾" => GetString("Fategiver"),
            "欺诈" or "欺诈者" or "欺诈师" => GetString("Fraudster"),
            "萧暮" or "暮" => GetString("XiaoMu"),
            "流浪人" or "流浪者" => GetString("Wanderers"),
            "秃鹫" => GetString("Vulture"),
            "雇佣兵" or "雇佣" or "投机者杀手" => GetString("OpportunistKiller"),
            "孟姜女" or "孟姜" => GetString("MengJiangGirl"),
            "受虐狂" or "受虐" or "抖m" or "抖M" => GetString("Masochism"),
            "公牛" or "牛" or "牛逼" or "一只牛" => GetString("Bull"),
            "修道徒" or "修道" => GetString("Cultivator"),
            "不屈" or "不屈者" => GetString("Indomitable"),
            "紊乱者" or "紊乱" => GetString("Disorder"),
            "行刑师" or "行刑" => GetString("OldImpostor"),
            "狼化者" or "狼化" => GetString("Attendant"),
            "恶臭先辈" or "哼哼啊啊啊" or "114514" or "恶臭之人" or "恶臭" => GetString("StinkyAncestor"),
            "侦察兵" or "侦查员" or "侦查兵" or "扫描人" => GetString("Scout"),
            "预言家" or "调查员" or "预言者" or "预言" => GetString("Prophet"),
            "捕快" or "辅警" => GetString("Deputy"),
            "正义的护盾师" or "护盾师" or "正义护盾" or "争议的护盾师" => GetString("NiceShields"),
            "银狼" => GetString("YinLang"),
            "失忆者" or "失忆" => GetString("Amnesiac"),
            "身份窃贼" or "窃贼" or "偷职业" or "交换师" or "交换" => GetString("Shifter"),
            "执行" or "执行者" => GetString("Executor"),
            "老千" or "出千" => GetString("OldThousand"),
            "狈" => GetString("Whoops"),
            ...
            "时间之主" or "时主" or "施主" or "失主" or "石柱" => GetString("TimeMaster"),
            "外星人" or "ET" => GetString("ET"),
            ...
            "魅魔" => GetString("Akujo"),
            "薛定谔的猫" or "猫" => GetString("SchrodingerCat"),
            "跟班" => GetString("Sidekick"),
            "外星人" or "外星" => GetString("ET"),
            "超能力者" or "超人强" or "超人" or "能力者" => GetString("SuperPowers"),
            "起诉人" or "起诉"  => GetString("Prosecutors"),
            "患者" or "病人" => GetString("Diseased"),
            "舰长" or "船长" or "贱长" => GetString("Captain"),
            "飞船干部" or "干部" or "舰干部" or "贱干部" or "叛变的坏批" => GetString("Solicited"),
            "病危者" or "Injured" or "Illness" => GetString("Injured"),
            "半兽人" => GetString("BSR"),
            "牛仔" => GetString("Cowboy"),
    ...
    public static void OnReceiveChat(PlayerControl player, string text, out bool canceled)
    {
        ...
        {
            Main.ChattyMax[player.PlayerId]++;
            if (Main.ChattyMax[player.PlayerId] >= Options.ChattyNumWin.GetInt())
            {
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Chatty);
        ...
        switch (args[0])
        {
          ...
          //      break;
            ...
            case "/s":
                ...
                {
                    if (args.Length > 1)
                        if (player.FriendCode.GetDevUser().Color == null && player.FriendCode.GetDevUser().Tag != null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={Main.ModColor}>开发者：{player.FriendCode.GetDevUser().Tag}的消息</color>】");

                        }
                        else if (player.FriendCode.GetDevUser().Tag == null && player.FriendCode.GetDevUser().Color != null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={player.FriendCode.GetDevUser().Color}>开发者：{player.name}的消息</color>】");

                        }
                        else if (player.FriendCode.GetDevUser().Tag == null && player.FriendCode.GetDevUser().Color == null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={Main.ModColor}>开发者：{player.name}的消息</color>】");
                ...
                else if (player.FriendCode.IsDevUser())
                {
                    if (args.Length > 1)
                    {
                        if (player.FriendCode.GetDevUser().Color == null && player.FriendCode.GetDevUser().Tag != null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={Main.ModColor}>金主/贡献者：{player.FriendCode.GetDevUser().Tag}的消息</color>】");

                        }
                        else if (player.FriendCode.GetDevUser().Tag == null && player.FriendCode.GetDevUser().Color != null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={player.FriendCode.GetDevUser().Color}>金主/贡献者：{player.name}的消息</color>】");

                        }
                        else if (player.FriendCode.GetDevUser().Tag == null && player.FriendCode.GetDevUser().Color == null)
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={Main.ModColor}>金主/贡献者：{player.name}的消息</color>】");

                        }
                        else
                        {
                            Utils.SendMessage(args.Skip(1).Join(delimiter: " "), title: $"【<color={player.FriendCode.GetDevUser().Color}>金主/贡献者：{player.FriendCode.GetDevUser().Tag}的消息</color>】");
