...
internal class ControllerManagerUpdatePatch
{
    ...
    public static void Postfix(ControllerManager __instance)//, PlayerControl player)
    {
        //切换自定义设置的页面
        ...
        if (Input.GetKeyDown(KeyCode.F2) && GameStates.InGame && Options.CurrentGameMode == CustomGameMode.Standard)
        {
            try
            {
                ...
                foreach (var subRole in Main.PlayerStates[lp.PlayerId].SubRoles.Where(x => x is not CustomRoles.seniormanagement))
                    addDes.Add(GetString($"{subRole}") + Utils.GetRoleMode(subRole) + GetString($"{subRole}InfoLong"));
                foreach (var subRole in Main.PlayerStates[lp.PlayerId].SubRoles.Where(x => x is not CustomRoles.Attendant))
                    addDes.Add(GetString($"{subRole}") + Utils.GetRoleMode(subRole) + GetString($"{subRole}InfoLong"));
        ...
        //-- 下面是主机专用的命令--//
        if (!AmongUsClient.Instance.AmHost)// || player.FriendCode.GetDevUser().IsDev)
        {
            return;
        }
        // 强制显示聊天框
