...
public static class Utils
{
    ...
    public static (string, Color) GetRoleText(byte seerId, byte targetId, bool pure = false)
    {
        ...
        if (targetSubRoles.Contains(CustomRoles.Attendant) && (self || pure || seerMainRole == CustomRoles.Jackal || (seerSubRoles.Contains(CustomRoles.Attendant))))
        {
    ...
    public static string GetProgressText(byte playerId, bool comms = false)
    {
        ...
        //    case CustomRoles.NiceTracker:
             //   ProgressText.Append(NiceTracker.GetMarker(playerId));
          //      break;
            ...
            case CustomRoles.Counterfeiter:
                ...
                break;
            case CustomRoles.Prophet:
                ProgressText.Append(Prophet.GetSkillLimit(playerId));
                break;
            case CustomRoles.Scout:
                ProgressText.Append(Scout.GetSkillLimit(playerId));
                break;
            case CustomRoles.Deputy:
                ProgressText.Append(Deputy.GetSkillLimit(playerId));
                break;
            case CustomRoles.Merchant:
                ProgressText.Append(Merchant.GetSkillLimit(playerId));
                break;
            case CustomRoles.Knight:
                ProgressText.Append(Knight.GetSkillLimit(playerId));
                break;
            case CustomRoles.QSR:
                ProgressText.Append(QSR.GetSkillLimit(playerId));
                break;
            case CustomRoles.DemonHunterm:
                ProgressText.Append(DemonHunterm.GetSkillLimit(playerId));
                break;
            case CustomRoles.Revolutionist:
            ...
            case CustomRoles.Vandalism:
                ProgressText.Append(Vandalism.GetVandalismLimit(playerId));
                break; 
            ...
            case CustomRoles.Succubus:
                ...
                break;
            case CustomRoles.Jackal:
                ProgressText.Append(Jackal.GetAttendantLimit(playerId)); 
                break;
            case CustomRoles.Captain:
                ProgressText.Append(Captain.GetSolicitLimit(playerId));
                break;
            case CustomRoles.ElectOfficials:
                ProgressText.Append(ElectOfficials.GetSkillLimit(playerId));
                break;
            ...
            case CustomRoles.Injured:
                ...
                ProgressText.Append(ColorString(GetRoleColor(CustomRoles.Injured), $"{Options.InjuredTurns.GetInt() - Main.DyingTurns}"));
                break;
            case CustomRoles.Strikers:
                ProgressText.Append(ColorString(GetRoleColor(CustomRoles.Strikers), $"{Main.StrikersShields}"));
                break;
            default:
                //タスクテキスト
    ...
    public static string GetSubRolesText(byte id, bool disableColor = false, bool intro = false, bool summary = false)
    {
        ...
        foreach (var role in SubRoles)
        {
            ...
            if (summary && role is CustomRoles.Madmate or CustomRoles.Charmed or CustomRoles.Attendant or CustomRoles.seniormanagement) continue;
    ...
    public static void ApplySuffix(PlayerControl player)
    {
        ...
        else
        {
            ...
            if (!name.Contains('\r') && player.FriendCode.GetDevUser().HasTag())
                ...
                {
                    SuffixModes.TOHE => name += $"\r\n<color={Main.ModColor}>{Main.ModName} v{Main.PluginVersion}</color>",
            ...
            if (!name.Contains('\r') && player.FriendCode.GetDevUser().HasTag())
                ...
                name = Options.GetSuffixMode() switch
                {
                    ...
                    SuffixModes.DoNotKillMe => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.DoNotKillMe")}</color></size>",
                    SuffixModes.NoAndroidPlz => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.NoAndroidPlz")}</color></size>",
                    SuffixModes.Test => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.Test")}</color></size>",
                    _ => name
                };
#endif
        }
    ...
    public static void NotifyRoles(bool isForMeeting = false, PlayerControl SpecifySeer = null, bool NoCache = false, bool ForceLoop = false, bool CamouflageisForMeeting = false, bool CamouflageIsActive = false)
    {
        ...
        foreach (var seer in seerList)
        {
            ...
            //如果是舰长
            if (seer.Is(CustomRoles.Captain))
                SelfMark.Append(ColorString(GetRoleColor(CustomRoles.Captain), " ★Cap★ "));

            //热！土！豆！
            if (seer.Is(CustomRoles.Hotpotato))
                SelfMark.Append(ColorString(GetRoleColor(CustomRoles.Hotpotato), "●"));
            ...
            if (seer.Is(CustomRoles.Vulture) && Vulture.VultureCanSeeDiePlayer.GetBool())
            {
         ...
         //   SelfSuffix.Append(NiceTracker.GetTargetArrow(seer, seer));
            ...
            foreach (var target in Main.AllPlayerControls)
            {
                ...
                if (target.Is(CustomRoles.Captain))
                    TargetMark.Append(ColorString(GetRoleColor(CustomRoles.Captain), " ★Cap★ "));

                //热！土！豆！
                if (target.Is(CustomRoles.Hotpotato))
                    TargetMark.Append(ColorString(GetRoleColor(CustomRoles.Hotpotato), "●"));
              ...
              //  if (seer.Is(CustomRoles.NiceTracker))
               // {
             //       TargetMark.Append(NiceTracker.GetTargetMark(seer, target));
              //  }
                ...
                if (seer.Is(CustomRoles.Strikers ))
                {
                    if (seer.IsAlive() && target.IsAlive() && isForMeeting)
                    {
                        TargetPlayerName = ColorString(GetRoleColor(CustomRoles.Strikers), target.PlayerId.ToString()) + " " + TargetPlayerName;
                    }
                }
    ...
    public static void AfterPlayerDeathTasks(PlayerControl target, bool onMeeting = false)
    {
            ...
            case CustomRoles.Captain:
                if (GameStates.IsMeeting)
                {
                    ...
                    foreach (var pc in Main.AllPlayerControls)
                    {
                        ...
                        SendMessage(string.Format(GetString("CaptainDead"), target.GetRealName()), pc.PlayerId, ColorString(GetRoleColor(CustomRoles.Captain), GetString("CaptainNewsTitle")));
                    }
                }
            else
            {
                if (!Main.CaptainDead.Contains(target.PlayerId))
        ...
        FixedUpdatePatch.HunterSuicide(target.PlayerId, onMeeting, true);
        FixedUpdatePatch.JackalSuicide(target.PlayerId, onMeeting, true);
        //FixedUpdatePatch.SheriffSuicide(target.PlayerId, onMeeting, true);
    }
    ...
    {
        string f = $"{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}/TOHEX-logs/";
        string t = DateTime.Now.ToString("yyyy-MM-dd_HH.mm.ss");
        string filename = $"{f}TOHEX-v{Main.PluginVersion}-{t}.log";
        if (!Directory.Exists(f)) Directory.CreateDirectory(f);
        FileInfo file = new(@$"{Environment.CurrentDirectory}/BepInEx/LogOutput.log");
        file.CopyTo(@filename);
        if (PlayerControl.LocalPlayer != null)
            HudManager.Instance?.Chat?.AddChat(PlayerControl.LocalPlayer, string.Format(GetString("Message.DumpfileSaved"), $"TOHEX - v{Main.PluginVersion}-{t}.log"));
    ...
    public static string ColorString(Color32 color, string str, int param1, int param2, int param3)
    {
        // 在这里处理额外的参数
        // 返回格式化后的字符串
        return $"<color=#{color.r:x2}{color.g:x2}{color.b:x2}{color.a:x2}>{str} ({param1}, {param2}, {param3})</color>";
    }
    /// <summary>
