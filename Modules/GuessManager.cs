...
public static class GuessManager
{
    ...
    public static bool GuesserMsg(PlayerControl pc, string msg, bool isUI = true)
    {
        var originMsg = msg;
        ...
        else if (operate == 2)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (!isUI) Utils.SendMessage(GetString("GuesserQL"), pc.PlayerId);
                    else pc.ShowPopUp(GetString("GuesserQL"));
                    return true;
                }
                if (pc.Is(CustomRoles.EvilGuesser) && pc.Is(CustomRoles.QL))
                {
                    if (!isUI) Utils.SendMessage(GetString("GuesserQL"), pc.PlayerId);
                    else pc.ShowPopUp(GetString("GuesserQL"));
                    return true;
                }
                if (role == CustomRoles.QL || target.Is(CustomRoles.QL))
                {
                    Utils.SendMessage(GetString("GuessQL"), pc.PlayerId);
                    return true;
                }
                ...
                {
                    Utils.SendMessage(GetString("GuessCaptain"), pc.PlayerId);
                    return true;
                }
                if (role == CustomRoles.Masochism || target.Is(CustomRoles.Masochism))
                {
                    Utils.SendMessage(GetString("GuessMasochism"), pc.PlayerId);
                    Main.MasochismKillMax[target.PlayerId]++;
                    pc.RPCPlayCustomSound("DM");
    ...
    static void GuesserOnClick(byte playerId, MeetingHud __instance)
    {
        ...
        try
        {
            ...
            Sprite namePlatrImage;
            AddressableAsset<NamePlateViewData> namePlateAsset = DestroyableSingleton<HatManager>.Instance.GetNamePlateById("nameplate_NoPlate").CreateAddressableAsset();
            namePlateAsset.LoadAsync();
            namePlatrImage = namePlateAsset.GetAsset().Image;
            ...
            for (int index = 0; index < 4; index++)
            {
                ...
                //Teambutton.GetComponent<SpriteRenderer>().sprite = DestroyableSingleton<HatManager>.Instance.GetNamePlateById("nameplate_NoPlate").CreateAddressableAsset();
            ...
            static void CreatePage(bool IsNext, MeetingHud __instance, Transform container)
            {
                ...
                namePlateAsset.LoadAsync();
                namePlatrImage = namePlateAsset.GetAsset().Image;
                //Pagebutton.GetComponent<SpriteRenderer>().sprite = DestroyableSingleton<HatManager>.Instance.GetNamePlateById("nameplate_NoPlate").CreateAddressableAsset();
            ...
            void CreateRole(CustomRoles role)
            {
                ...
                //button.GetComponent<SpriteRenderer>().sprite = DestroyableSingleton<HatManager>.Instance.GetNamePlateById("nameplate_NoPlate").CreateAddressableAsset();
